...
extern "C" float* launch_gpu_computation(float *image, int i_width, int i_height, float *temp, int t_width) {
    /* Mallocing the gpu_image which is the image that will be put into the calc_min_dist file. */
    float* gpu_image;
    ...
    /* Mallocing the template for use in the the calc_min_dist calculation file. */
    float* gpu_temp;
    ...
    /* Freeing the template and image */
    cudaFree(gpu_image);
    cudaFree(gpu_temp);
...
extern "C" float just_get_dist_gpu(float *image, int i_width, int i_height, float *temp, int t_width) {
    float* res;
    printf("printing in just_get_dist_gpu");
